...
class SettingController extends Controller
{
  ...
  public function listCategory(Request $request)
    ...
    $business_types = Mst_business_types::all();
			...
			$categories = Mst_categories::where('business_type_id', $business_type_id )->get();
...
public function destroyAgency(Request $request,Mst_store_agencies $agency)
   	 ...
   	 public function assignAgency(Request $request, $id)
    {
		...
		$agencies = Mst_store_agencies::all();
		$linked_agencies = Mst_store_link_agency::all();
//dd($agencies);
...
public function listCustomer(Request $request)
{
    ...
    public function viewcustomer(Request $request, $id)
    {
		...
		return view('admin.masters.customers.view',compact('customers','pageTitle','countries'));
  ...
  public function updatecustomer(Request $request,Trn_store_customer $customer, $customer_id)
    {
		 ...
		 $newpassword = $request->customer_password;
            ...
            [
                ...
                'customer_password'          => 'sometimes|same:password_confirmation',
...
->select("mst_delivery_boys.*");
		...
		if(isset($request->date_from) && isset($request->date_to))
		{
		    $query = $query->whereBetween('mst_delivery_boys.created_at',[$a1,$a2]);
		}
...
DB::enableQueryLog();
   ...
   // $delivery_Boy = Mst_delivery_boy::Find($decrId);
    //delivery_boy_id
 	return view('admin.masters.order.view',compact('order','pageTitle','status','customer'));
 		} catch (\Exception $e) {
    ...
    $status = Sys_store_order_status::all();
 	...
 	return view('admin.masters.order.invoice',compact('order','pageTitle','status','customer'));
		} catch (\Exception $e) {
			//echo $e->getMessage();die;
			return redirect()->back()->withErrors(['Something went wrong!'])->withInput();
		}
...
public function list_delivery_boys_payment_settlment(Request $request)
{
	...
	if($_GET){
		   ...
		   $delivery_boy_id = $request->delivery_boy_id;
		   $delivery_boys = Mst_delivery_boy::where('delivery_boy_id', 'like', '%'.$delivery_boy_id.'%')->get();

		 //  $delivery_boy_payments = Trn_delivery_boy_payment_settlment::where('delivery_boy_id', 'like', '%'.$delivery_boy_id.'%')->get();


	return view('admin.masters.delivery_boys_payment.list',compact('pageTitle','delivery_boys'));
			   }
